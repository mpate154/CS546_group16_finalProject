...
document.getElementById('edit_info_form').addEventListener('submit', async function(event) {
  ...
  const balance = document.getElementById('balance').value.trim();
    ...
    if (!firstName || firstName.length < 2 || firstName.length > 20 || !/^[a-zA-Z]+$/.test(firstName)) {
      showFormError('First Name must be 2-20 characters long and only contain letters.','edit_info_form');

      return;
    }

    if (!lastName || lastName.length < 2 || lastName.length > 20 || !/^[a-zA-Z]+$/.test(lastName)) {
      showFormError('Last Name must be 2-20 characters long and only contain letters.','edit_info_form');
      return;
    }
    ...
    if (!balance || isNaN(balance) || parseFloat(balance) < 0) {
      showFormError('Balance should be a valid positive number.','edit_info_form');
      return;
    }
  ...
  if (response.ok) {
    document.querySelector('#user_info p:nth-child(1)').innerText = `First Name: ${firstName}`;
    document.querySelector('#user_info p:nth-child(2)').innerText = `Last Name: ${lastName}`;
    document.querySelector('#user_info p:nth-child(3)').innerText = `Email: ${email}`;
    document.querySelector('#user_info p:nth-child(4)').innerText = `Gender: ${gender}`;
    document.querySelector('#user_info p:nth-child(5)').innerText = `City: ${city}`;
    document.querySelector('#user_info p:nth-child(6)').innerText = `State: ${state}`;
    document.querySelector('#user_info p:nth-child(7)').innerText = `Age: ${age}`;
    document.querySelector('#user_info p:nth-child(8)').innerText = `Balance: ${balance}`;
...
document.getElementById('add_category_form').addEventListener('submit', async function(event) {
  event.preventDefault();
  ...
  if (response.ok) {
    ...
    li.querySelector('.delete-category').addEventListener('click', async function() {
...
document.querySelectorAll('.delete-category').forEach(button => {
  button.addEventListener('click', async function() {
    ...
    const optionElement = document.querySelector(`#fixedCategory option[value="${category}"]`);
...
document.getElementById('add_fixed_expense_form').addEventListener('submit', async function(event) {
  event.preventDefault();
  ...
  try {
    ...
    if (response.ok) {
      ...
      showFormError(`Error: ${errorResponse.error}`, 'add_fixed_expense_form');
    }
  } catch (error) {
    console.error(error);
    showFormError('An error occurred while adding the expense.', 'add_fixed_expense_form');
  }

});

// Edit and Delete Fixed Expenses
document.getElementById('fixed_expense_list').addEventListener('click', async function(event) {
  ...
  if (target.classList.contains('edit-expense')) {
    ...
    const amount = li.querySelector('.expense-amount').innerText.replace('$', '');
  ...
  if (target.classList.contains('save-expense')) {
    ...
    if (response.ok) {
      ...
      li.innerHTML = `
        ...
        <span class="expense-amount">$${updatedExpense.amount.toFixed(2)}</span>
        <button class="edit-expense">✏️</button>
        <button class="delete-expense">❌</button>
      `;
    } else {
  ...
  if (target.classList.contains('cancel-edit')) {
    ...
    li.innerHTML = `
      ...
      <span class="expense-amount">$${originalExpense.amount.toFixed(2)}</span>
      <button class="edit-expense">✏️</button>
      <button class="delete-expense">❌</button>
    `;
  }
